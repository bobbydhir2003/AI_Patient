...
class User(Base):
    ...
    # LEGACY / no-op. The application now has exactly two roles (student, admin)
    # and every admin has all admin powers, so this column no longer affects
    # authorization or routing. It is retained only for backward compatibility
    # with the existing production schema; nothing reads it for access control.
