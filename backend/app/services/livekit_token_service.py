...
def student_room_name(session_id: str) -> str:
    """Same server-derivation contract as poc_room_name, distinct prefix -
    see api/interviews.py's ownership check BEFORE this is ever called."""
    return f"{_STUDENT_ROOM_PREFIX}{session_id}"
...
def _mint_token(*, user: User, session: InterviewSession, room_name: str) -> PocTokenResult:
...
def create_student_token(*, user: User, session: InterviewSession) -> PocTokenResult:
    ...
    if not student_livekit_enabled():
        raise LiveKitNotConfiguredError()
    return _mint_token(user=user, session=session, room_name=student_room_name(session.id))
